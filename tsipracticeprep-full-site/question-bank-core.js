...
function renderNav(){
 ...
 // one long row; the column count is set here from the real question count
 // so numbers read left-to-right, top row then bottom row, in order.
 navEl.style.gridTemplateColumns=`repeat(${Math.ceil(state.set.length/2)},32px)`;
 navEl.querySelector(".current")?.scrollIntoView({inline:"center",block:"nearest"});
}
...
function renderGeneric(state,prefix,store,saveFn,renderFn){
 ...
 const navEl=$(prefix+"Navigator");
 navEl.innerHTML=state.set.map((x,i)=>`<button class="navq ${state.answers[x.id]!==undefined?"answered":""} ${i===state.i?"current":""} ${state.flags.includes(x.id)?"flagged":""}" data-${prefix.toLowerCase()}-jump="${i}">${i+1}</button>`).join("");
 ...
 // Same 2-row navigator grid as TSIA2's renderNav() (see
 // .practiceShell.show .navigator) -- SAT (98) and ACT (131) have even more
 // questions than TSIA2, so showing two rows at once matters even more here.
 navEl.style.gridTemplateColumns=`repeat(${Math.ceil(state.set.length/2)},32px)`;
 navEl.querySelector(".current")?.scrollIntoView({inline:"center",block:"nearest"});
}
...
function showCalculator(exam,q){
 if(exam==="tsi")$("calculatorNotice").textContent="TSIA2 permits an on-screen calculator on some Mathematics questions. This TSI Practice Prep calculator is a practice tool and appears only on questions tagged calculator-allowed.";
 if(exam==="sat")$("calculatorNotice").textContent="Digital SAT Math allows calculator use. This TSI Practice Prep calculator is a practice implementation, not College Board's Bluebook calculator.";
 if(exam==="act")$("calculatorNotice").textContent="ACT allows a permitted calculator on the Mathematics test. This TSI Practice Prep calculator is a practice implementation, not an ACT-provided calculator.";
...
function showReference(exam,q){
 ...
 else $("referenceNotice").textContent="This is a TSI Practice Prep study reference for the current math topic. It is not presented as an official universal reference sheet for this exam.";
